...
import { ReportGetRequest, ReportPostRequest } from "../../api/Report_Request";
import { SpecializationGetRequest } from "../../api/Specialization_Request";
import { HospitalGetRequest } from "../../api/Hospital_Request";
...
const AddModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (Number(fromage) >= Number(toage)) {
      ...
      const json = await ReportPostRequest({
        FK_hospital_ID: referringHospital,
        FK_specializations_ID: servicetype,
        FK_user_ID: 1, //put id of authenticated user
        report_AgeFrom: fromage,
        report_AgeTo: toage,
        report_DateFrom: fromdate,
        report_DateTo: todate,
        report_Sex: sex,
      });
...
const Report = () => {
  ...
  const handleFetchReport = async () => {
    const res = await ReportGetRequest();
  ...
  const serviceTypeData = async () => {
    ...
    const jsonHospital = await HospitalGetRequest({ params: {} });
    if (jsonHospital === null) {
      return;
    }
    setHospitalData(jsonHospital.data.data);
  };
